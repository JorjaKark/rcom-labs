                       ...
                       unsigned char expectedC)
{
    ...
    switch (*state) {
    case ST_START:
        if (incomingByte == FLAG) *state = ST_FLAG;
        break;
...
{
    int outLen = 0;
    for (int i = 0; i < n; ++i) outLen += stuff_byte(in[i], out + outLen);
    return outLen;
}
...
static int build_iframe(unsigned char address,
                        ...
                        int outCapacity)
{
    if (dataLen < 0 || dataLen > MAX_IFRAME_DATA) return -1;

    int worstCase = 1 + 3 + (2 * dataLen) + 2 + 1;
    if (outCapacity < worstCase) return -1;
    ...
    if (dataLen > 0) pos += stuff_bytes(data, dataLen, out + pos);
...
{
    enum { ST_S, ST_F, ST_A, ST_C, ST_B } state = ST_S;
    ...
    for (;;) {
        ...
        switch (state) {
        case ST_S:
            if (byte == FLAG) state = ST_F;
            break;

        case ST_F:
            if (byte == expectedAddress) { A = byte; state = ST_A; }
            else if (byte != FLAG) { state = ST_S; }
            break;
                       ...
                       int maxDataLen)
{
    ...
    while (1) {
        ...
        if (r == 0) continue;
        if (byte == FLAG) break;
        if (bodyLen >= (int)sizeof(body)) return -1;
        body[bodyLen++] = byte;
    }
    ...
    if (computedBcc2 != bcc2) return -1;
    if (payloadLen > maxDataLen) return -1;

    if (payloadLen > 0) memcpy(dataOut, destuffed, payloadLen);
    if (controlOut) *controlOut = C;
...
int llopen(LinkLayer connectionParameters)
{
    ...
    if (connectionParameters.role == LlTx) {
        ...
        while (1) {
            int r = readByteSerialPort(&incomingByte);
            if (r < 0) { closeSerialPort(); return -1; }
            if (r == 0) continue;
...
int llread(unsigned char *appPacketOut)
{
    for (;;) {
        ...
        if (payloadLen < 0) {
            if (send_rej(g_rxExpected) < 0) return -1;
            continue;
        }
        ...
        if (ns == g_rxExpected) {
            if (send_rr(g_rxExpected ^ 1) < 0) return -1;
            g_rxExpected ^= 1;
            return payloadLen;
        } else {
            if (send_rr(g_rxExpected) < 0) return -1;
        }
    }
}
